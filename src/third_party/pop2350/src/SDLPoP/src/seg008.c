...
void draw_tile_anim_right() {
	switch (tile_left) {
		...
		case tiles_30_torch_with_debris:
			if (modifier_left < 9) {
				// Clear the flame area first to remove previous frame
				add_wipetable(0, (draw_xh + 1) * 8, draw_main_y - 40, 45, 24, 0); // Clear 24x45px area with black
				
				int blit = blitters_0_no_transp; // Revert to non-transparent to overwrite previous frame
...
void draw_tile_fore() {
	...
	switch (curr_tile) {
		...
		case tiles_20_wall:
			if (custom->tbl_level_type[current_level] == 0 || custom->enable_wda_in_palace || graphics_mode != gmMcgaVga) {
				// Draw wall even with modifier 0 - it's still a wall tile
				int wall_variant = curr_modifier & 0x03; // low bits store adjacency info
				int frame_id = wall_fram_main[wall_variant];
				add_foretable(id_chtab_7_environmentwall, frame_id, draw_xh, 0, draw_main_y, blitters_0_no_transp, 0);
			}
...
image_type* get_image(short chtab_id, int id) {
	...
	if (image == NULL) {
        // printf("seg008: calling reload_streamed_image(chtab=%p, id=%d)\n", chtab, id);
        fflush(stdout);
		image = reload_streamed_image(chtab, id);
        // printf("seg008: reload returned %p\n", image);
        fflush(stdout);
		if (image == NULL) {
			printf("Failed to load streamed image %d of chtab %d\n", id, chtab_id);
			return NULL;
		}
	}
	return image;
...
int add_backtable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom, int blit, byte peel) {
	...
	draw_image(image, mask, backtable_item->xh * 8 + backtable_item->xl, backtable_item->y, blit);
	++backtable_count;
	return 1;
}
...
int add_foretable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom, int blit, byte peel) {
	...
	image_type* mask = image;
	draw_image(image, mask, foretable_item->xh * 8 + foretable_item->xl, foretable_item->y, blit);
	++foretable_count;
	return 1;
}
...
int add_midtable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom, int blit, byte peel) {
	...
	// Immediate drawing removed - deferred to draw_tables() for non-blocking operation
	++midtable_count;
	return 1;
}
...
void add_peel(int left,int right,int top,int height) {
	...
	// Clip peel to game area (y < 192) to prevent artifacts on health bar
	...
	if (height <= 0) {
		return; // Peel is entirely in health bar area, skip it
	}
	
	//printf("add_peel: peels_count=%d, rect=(%d,%d,%d,%d)\n", peels_count, left, top, right, top+height);
	rect.left = left;
	rect.right = right;
	rect.top = top;
	rect.bottom = bottom;
...
void draw_back_fore(int which_table,int index) {
	...
	*/
	// Set clip to game area to prevent drawing into status bar (y>=192)
	rect_type game_clip = {0, 0, 192, 320};
	set_clip_rect(&game_clip);
...
//	word image_flipped;
	...
	*/
	...
	// Always set clip rect to game area to prevent drawing into status bar (y>=192)
	// Use fixed game area clip instead of per-sprite clip to avoid garbage values
	rect_type game_clip = {0, 0, 192, 320};
	set_clip_rect(&game_clip);
	...
	if (blit_flip) {
        ...
        saved_offset = psram_get_temp_offset();
	...
	// Always reset clip rect since we always set it now
	reset_clip_rect();
...
void draw_image(image_type* image,image_type* mask,int xpos,int ypos,int blit) {
    //printf("draw_image: image=%p, mask=%p, x=%d, y=%d, blit=%d\n", image, mask, xpos, ypos, blit);
    //fflush(stdout);
	rect_type rect;
	switch (blit) {
...
void draw_gate_back() {
	calc_gate_pos();
	if (gate_bottom_y < gate_top_y) {
		// Gate fully open - just draw wall
		...
		// Gate visible - draw wall and gate bars
		// Clear the gate area to remove old pixels from previous frame
		add_wipetable(0, draw_xh * 8, draw_main_y, 63, 32, 0);
		add_backtable(id_chtab_6_environment, tile_table[tiles_4_gate].right_id, draw_xh, 0,
		              tile_table[tiles_4_gate].right_y + draw_main_y, blitters_0_no_transp, 0);
		if (can_see_bottomleft()) draw_tile_topright();
		...
		draw_loose(0);
		draw_tile_base();
		// Draw gate bars - bottom piece first (transparent background)
		add_backtable(id_chtab_6_environment, 51 /*gate bottom*/, draw_xh, 0, gate_bottom_y - 2, blitters_10h_transp, 0);
		// Then draw vertical slices moving upward (opaque blitter)
		short ybottom = gate_bottom_y - 12;
		if (ybottom < 192) {
			while (ybottom > gate_top_y) {
...
void draw_gate_fore() {
	...
	short ybottom = gate_bottom_y - 12;
	if (ybottom < 192) {
		// Keep drawing until we reach gate_top_y
		while (ybottom > gate_top_y) {
			add_foretable(id_chtab_6_environment, 52 /*gate slice 8px*/, draw_xh, 0, ybottom, blitters_0_no_transp, 0);
			ybottom -= 8;
		}
	}
}
...
// seg008:1AF8
void draw_moving() {
	// Set target surface and clip rect to game area (y < 192) to prevent drawing into health bar
	current_target_surface = offscreen_surface;
	rect_type game_area_clip = {0, 0, 192, 320};
	set_clip_rect(&game_area_clip);
...
void draw_tables() {
	drects_count = 0;
	current_target_surface = offscreen_surface;
	reset_blit_cache(); // Reset SDL state cache for this frame
	
	// Set clip rect to game area (y < 192) to prevent drawing into health bar
	rect_type game_area_clip = {0, 0, 192, 320};
	set_clip_rect(&game_area_clip);
	...
	// Reset clip rect to allow health bar drawing
	reset_clip_rect();
...
void draw_people() {
	...
	// Temporarily reset clip rect to allow HP bar drawing (y >= 192)
	reset_clip_rect();
	draw_hp();
	// Restore clip rect for game area (y < 192)
	rect_type game_area_clip = {0, 0, 192, 320};
	set_clip_rect(&game_area_clip);
}
...
void wall_pattern(int which_part,int which_table) {
	...
	if ( (!is_dungeon) && (graphics_mode== GRAPHICS_VGA) ) {
		...
		// low two bits only describe adjacency; higher bits encode fake wall metadata
		byte bg_modifier = curr_modifier & 0x03;
		switch (bg_modifier) {
