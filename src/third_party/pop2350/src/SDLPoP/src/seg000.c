...
static image_type* ensure_chtab_image(chtab_type* chtab, int id) {
	if (!chtab) return NULL;
	if (id < 0 || id >= chtab->n_images) return NULL;
	image_type* image = chtab->images[id];
	if (image == NULL) {
        printf("ensure_chtab_image: calling reload_streamed_image(chtab=%p, id=%d)\n", chtab, id);
        fflush(stdout);
		image = reload_streamed_image(chtab, id);
        printf("ensure_chtab_image: reload returned %p\n", image);
        fflush(stdout);
	}
	return image;
}
...
void pop_main() {
    ...
    printf("Calling open_dat(PRINCE.DAT)\n");
	dathandle = open_dat("PRINCE.DAT", 'G');
    debug_blink(5);
    printf("open_dat(PRINCE.DAT) returned %p\n", dathandle);
...
// seg000:024F
void init_game_main() {
    printf("init_game_main: Started\n");
	doorlink1_ad = /*&*/level.doorlinks1;
	doorlink2_ad = /*&*/level.doorlinks2;
	prandom(1);
	if (graphics_mode == gmMcgaVga) {
        printf("init_game_main: Loading guard palettes\n");
		// Guard palettes
        ...
        printf("init_game_main: Loading level variations\n");
		// Level color variations (1.3)
		level_var_palettes = load_from_opendats_alloc(20, "bin", NULL, NULL);
	}
    printf("init_game_main: Loading sword sprites\n");
	const char* prince_dat_name = dathandle ? dathandle->filename : "PRINCE.DAT";
	// PRINCE.DAT: sword
	chtab_addrs[id_chtab_0_sword] = load_sprites_from_file(700, 1<<2, 1, prince_dat_name);
    printf("init_game_main: Loading flame/potion sprites\n");
	// PRINCE.DAT: flame, sword on floor, potion
	chtab_addrs[id_chtab_1_flameswordpotion] = load_sprites_from_file(150, 1<<3, 1, prince_dat_name);
    printf("init_game_main: Closing DAT\n");
	close_dat(dathandle);
    printf("init_game_main: DAT closed\n");
#ifdef USE_LIGHTING
	init_lighting();
    printf("init_game_main: Lighting initialized\n");
#endif
    printf("init_game_main: Loading sounds\n");
	load_all_sounds();
    printf("init_game_main: Sounds loaded\n");

	hof_read();
    printf("init_game_main: HOF read\n");
    printf("init_game_main: Showing splash\n");
	show_splash(); // added
    printf("init_game_main: Splash shown\n");
	
	// Mark PSRAM state after initialization - we'll restore to this point between games
	psram_mark_session();
	
    printf("init_game_main: Starting game\n");
	start_game();
}
...
// seg000:0358
void start_game() {
    static int very_first_call = 1; // Track if this is the very first invocation
    printf("start_game: Started\n");
#ifdef USE_COPYPROT
	word which_entry;
	word entry_used[40];
	byte letts_used[26];
	...
	// Only clean up and restore session on subsequent starts (after death/restart)
	// Skip this on the very first call to start_game
	if (!very_first_call) {
		printf("start_game: Cleaning up session (very_first_call=%d)\n", very_first_call);
		release_title_images(); // added
		free_optsnd_chtab(); // added
		
		// Restore PSRAM to marked session point - reclaims all game allocations
		// while preserving SDL initialization and permanent resources
		printf("start_game: Calling psram_restore_session()\n");
		psram_restore_session();
		printf("start_game: Session restored\n");
		
		// NULL out global pointers that point into freed game session memory
		offscreen_surface = NULL;
		current_target_surface = NULL;
	}
	very_first_call = 0;
	...
	if (custom->skip_title) { // CusPop option: skip the title sequence (level loads instantly)
        ...
        printf("start_game: init_game(%d)\n", level_number);
		init_game(level_number);
		return;
	}

	if (start_level < 0) {
        printf("start_game: show_title()\n");
		show_title();
	} else {
        printf("start_game: init_game(%d)\n", start_level);
	...
	if (need_full_redraw) {
		...
		if (different_room) {
			...
			if (need_redraw_because_flipped) {
				...
				// Fix: Clear and redraw HP bar after all drawing to remove any artifacts
				current_target_surface = offscreen_surface;
				reset_clip_rect();
				rect_type rect_hp_clear = {192, 0, 200, 320};
				draw_rect(&rect_hp_clear, color_0_black);
				...
				// Force copy HP bar area to onscreen surface to ensure no artifacts
				copy_screen_rect(&rect_hp_clear);
				if (upside_down) {
					flip_screen(offscreen_surface);
				}
				drects_count = 0;
...
void load_lev_spr(int level) {
	...
	if (current_target_surface == NULL) {
		printf("ERROR: current_target_surface is NULL in load_lev_spr!\n");
...
void draw_kid_hp(short curr_hp,short max_hp) {
	...
	// Ensure the HP bar is updated on screen
	rect_type rect_hp = {194, 0, 205, (short)(7 * max_hp)};
	add_drect(&rect_hp);
}
...
void draw_guard_hp(short curr_hp,short max_hp) {
	...
	if (guard_charid != charid_4_skeleton &&
		...
		image_type* hp_image = get_image(id_chtab_5_guard, 0);
		if (hp_image == NULL) return;
		for (short drawn_hp_index = curr_hp; drawn_hp_index < max_hp; ++drawn_hp_index) {
			method_6_blit_img_to_scr(hp_image, 314 - drawn_hp_index * 7, 194, blitters_9_black);
		}
		for (short drawn_hp_index = 0; drawn_hp_index < curr_hp; ++drawn_hp_index) {
			method_6_blit_img_to_scr(hp_image, 314 - drawn_hp_index * 7, 194, blitters_0_no_transp);
		}
		// Ensure the Guard HP bar is updated on screen
		rect_type rect_hp = {194, (short)(314 - max_hp * 7), 205, 320};
		add_drect(&rect_hp);
	}
}
...
void draw_hp() {
	...
	// Clear the health bar area first to prevent garbage
	rect_type rect_hp_clear = {192, 0, 200, 320};
	draw_rect(&rect_hp_clear, color_0_black);

	if (hitp_delta || 1) { // Force redraw to ensure it's always visible
		draw_kid_hp(hitp_curr, hitp_max);
		drawn = 1;
	}
	...
	if (guardhp_delta || 1) { // Force redraw
		draw_guard_hp(guardhp_curr, guardhp_max);
		drawn = 1;
	}
	if (guardhp_curr == 1) {
	...
	if (drawn) {
		rect_type rect_hp = {192, 0, 200, 320};
		add_drect(&rect_hp);
	}
}
...
void load_chtab_from_file(int chtab_id,int resource,const char* filename,int palette_bits) {
	...
	const char* dat_name = dathandle ? dathandle->filename : filename;
	chtab_addrs[chtab_id] = load_sprites_from_file(resource, palette_bits, 1, dat_name);
	close_dat(dathandle);
}
...
void read_keyb_control() {
	...
	if (key_states[SDL_SCANCODE_UP] & key_state || key_states[SDL_SCANCODE_HOME] & key_state || key_states[SDL_SCANCODE_PAGEUP] & key_state
	    ...
	    || key_states[key_up] & key_state || key_states[key_jump_left] & key_state || key_states[key_jump_right] & key_state
	) {
		control_y = CONTROL_HELD_UP;
	...
	if (key_states[SDL_SCANCODE_LEFT] & key_state || key_states[SDL_SCANCODE_HOME] & key_state
	    ...
	    || key_states[key_left] & key_state || key_states[key_jump_left] & key_state
	) {
		control_x = CONTROL_HELD_LEFT;
	           ...
	           || key_states[key_right] & key_state || key_states[key_jump_right] & key_state
	) {
		control_x = CONTROL_HELD_RIGHT;
	}
	
	if(key_states[SDL_SCANCODE_LSHIFT] & key_state || key_states[SDL_SCANCODE_RSHIFT] & key_state || key_states[key_action] & key_state)
		control_shift = CONTROL_HELD;
	else
		control_shift = CONTROL_RELEASED;
...
void show_title() {
    ...
    printf("show_title: load_title_images\n");
	load_title_images(1);
	current_target_surface = offscreen_surface;
    printf("show_title: idle\n");
	idle(); // modified
	do_paused();

    printf("show_title: draw_full_image(TITLE_MAIN)\n");
	draw_full_image(TITLE_MAIN);
    printf("show_title: fade_in_2\n");
	fade_in_2(offscreen_surface, 0x1000); //STUB
    printf("show_title: method_1_blit_rect\n");
    ...
    printf("show_title: play_sound_from_buffer\n");
	play_sound_from_buffer(sound_pointers[sound_54_intro_music]); // main theme
    printf("show_title: start_timer\n");
...
void draw_full_image(enum full_image_id id) {
	...
	chtab_type* chtab = *full_image[id].chtab;
	decoded_image = ensure_chtab_image(chtab, full_image[id].id);
	if (decoded_image == NULL) return;
	
	// Debug print
	printf("draw_full_image: id=%d, decoded_image=%p, w=%d, h=%d\n", id, decoded_image, decoded_image->w, decoded_image->h);
    fflush(stdout);
...
void load_title_images(int bgcolor) {
	...
	const char* title_name = dathandle ? dathandle->filename : "TITLE.DAT";
	chtab_title40 = load_sprites_from_file(40, 1<<11, 1, title_name);
	chtab_title50 = load_sprites_from_file(50, 1<<12, 1, title_name);
	close_dat(dathandle);
	if (graphics_mode == gmMcgaVga) {
		...
		if (NULL != chtab_title40) {
			image_type* frame = ensure_chtab_image(chtab_title40, 0);
			if (frame && frame->format && frame->format->palette) {
				SDL_SetPaletteColors(frame->format->palette, &color, 14, 1);
				// Also update the screen palette to match, to avoid palette mapping artifacts
				if (onscreen_surface_ && onscreen_surface_->format && onscreen_surface_->format->palette) {
					SDL_SetPaletteColors(onscreen_surface_->format->palette, &color, 14, 1);
				}
			}
		}
