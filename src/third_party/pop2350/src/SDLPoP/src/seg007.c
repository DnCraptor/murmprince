...
void animate_torch() {
	...
	if (trob.room == drawn_room || (trob.room == room_L && (trob.tilepos % 10) == 9) ) {
		...
		// Use anim redraw instead of full - wipe already clears the flame area
		// This avoids recreating sprites/peels on every frame
...
*/
	...
	// OPTIMIZATION: Call draw_trob() to mark tiles for redraw
	// draw_trob() now conditionally skips set_wipe() for gates to avoid keyboard freeze
	draw_trob();
}
...
void draw_mob() {
	short ypos = curmob.y;
	if (curmob.room == drawn_room) {
		if (curmob.y >= 192) return; // Clip to game area, prevent artifacts on health bar
...
void add_mob_to_objtable(int ypos) {
	...
	curr_obj->clip.bottom = 192; // Clip to game area, prevent artifacts on health bar
