...
void init_game(int level) {
    // printf("init_game: Started\n");
	if(offscreen_surface) {
		// Clear before freeing to prevent artifacts
		current_target_surface = offscreen_surface;
		draw_rect(&screen_rect, color_0_black);
		free_surface(offscreen_surface);
		offscreen_surface = NULL;
	}
	// Use screen_rect (200 tall) instead of rect_top (192 tall) to include status bar area
	offscreen_surface = make_offscreen_buffer(&screen_rect);
	// Clear offscreen surface to black to avoid garbage pixels
	current_target_surface = offscreen_surface;
	draw_rect(&screen_rect, color_0_black);
	// Also clear the onscreen surface to remove title screen remnants
	current_target_surface = onscreen_surface_;
	draw_rect(&screen_rect, color_0_black);
    // printf("init_game: load_kid_sprite\n");
	load_kid_sprite();
...
void play_level(int level_number) {
	...
	for (;;) {
		...
		if (level_number != current_level) {
            // printf("play_level: load_lev_spr(%d)\n", level_number);
			load_lev_spr(level_number);
		}
        // printf("play_level: load_level()\n");
		load_level();
        // printf("play_level: pos_guards()\n");
		pos_guards();
		// printf("play_level: clear_coll_rooms()\n");
		clear_coll_rooms();
		// printf("play_level: clear_saved_ctrl()\n");
		clear_saved_ctrl();
		// printf("play_level: setting drawn_room\n");
		drawn_room = 0;
		mobs_count = 0;
		trobs_count = 0;
		next_sound = -1;
		...
		// printf("play_level: find_start_level_door()\n");
		find_start_level_door();
		// busy waiting?
		// printf("play_level: check_sound_playing loop...\n");
		while (check_sound_playing() && !do_paused()) idle();
		// printf("play_level: stop_sounds()\n");
		stop_sounds();
		#ifdef USE_REPLAY
		...
		#endif
		// printf("play_level: draw_level_first()\n");
		draw_level_first();
		// printf("play_level: show_copyprot()\n");
		show_copyprot(0);
		// printf("play_level: entering play_level_2()\n");
...
void draw_level_first() {
	// printf("draw_level_first: setting next_room\n");
	fflush(stdout);
	next_room = Kid.room;
	
	// Clear all peels when starting gameplay to remove title screen artifacts
	clear_all_peels();
	
	printf("draw_level_first: check_the_end()\n");
	fflush(stdout);
	check_the_end();
	if (custom->tbl_level_type[current_level]) {
		printf("draw_level_first: gen_palace_wall_colors()\n");
		fflush(stdout);
		gen_palace_wall_colors();
	}
	printf("draw_level_first: draw_rect()\n");
	fflush(stdout);
	draw_rect(&screen_rect, color_0_black);
	printf("draw_level_first: show_level()\n");
	fflush(stdout);
	show_level();
	printf("draw_level_first: redraw_screen()\n");
	fflush(stdout);
	redraw_screen(0);
	printf("draw_level_first: draw_kid_hp()\n");
	fflush(stdout);
	...
	printf("draw_level_first: start_timer()\n");
	// Busy waiting!
	start_timer(timer_1, 5);
	printf("draw_level_first: do_simple_wait()\n");
	do_simple_wait(1);
	printf("draw_level_first: done\n");
}
...
void redraw_screen(int drawing_different_room) {
	...
	// Clear peel pool when changing rooms to prevent stale artifacts
	...
	if (drawing_different_room) {
		...
		// Also clear the bottom status bar area to prevent garbage
		rect_type rect_bottom = {192, 0, 200, 320};
		draw_rect(&rect_bottom, color_0_black);
		
		// Force redraw of HP bars
		draw_kid_hp(hitp_curr, hitp_max);
	...
	if (is_blind_mode) {
		...
		need_drects = 0;
		// Clear tables before populating them to avoid garbage from previous frame
		memset(&table_counts, 0, sizeof(table_counts));
		redraw_room();
		// Set clip rect to game area before drawing moving objects
		rect_type game_area_clip = {0, 0, 192, 320};
		set_clip_rect(&game_area_clip);
		// Populate tables with moving objects (Kid, Guard, HP bar) for the new room
		draw_mobs();
		draw_people();
#ifdef USE_LIGHTING
	redraw_lighting();
