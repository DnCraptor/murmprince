...
void play_level(int level_number) {
	...
	for (;;) {
		...
		#endif
#ifdef POP_RP2350
		// Reset HDMI fade to full brightness before drawing gameplay
		// (cutscene may have left it at black)
...
void draw_level_first() {
	...
	redraw_screen(0);
#ifdef POP_RP2350
	// Reset clip rect to allow HP drawing at y=194
...
#ifdef POP_RP2350
	// Force screen update to show HP bar on first screen
	update_screen();
#endif
