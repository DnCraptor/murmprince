...
void add_peel(int left,int right,int top,int height) {
	...
	// Clip peel to screen bounds to prevent out-of-bounds access
	int bottom = top + height;
	
	// Clip to screen width (0-320)
	if (left < 0) left = 0;
	if (right > 320) right = 320;
	if (left >= right) {
		return; // Zero or negative width after clipping
	}
	
	// Clip to screen height (0-192 for game area)
	if (top < 0) top = 0;
	if (bottom > 192) bottom = 192;
	if (top >= bottom) {
		return; // Peel is entirely in health bar area or off-screen, skip it
	}
...
SDL_Surface* hflip(SDL_Surface* input) {
	int width = input->w;
	int height = input->h;
	int bpp = input->format->BytesPerPixel;

#ifdef POP_RP2350
	// Use SRAM mode so this surface can be properly freed later
	psram_set_sram_mode(1);
	
	// Memory-efficient version: create surface without palette, adopt input's palette
	Uint32 flags = (bpp == 1) ? SDL_NO_PALETTE : 0;
	SDL_Surface* output = SDL_CreateRGBSurface(flags, width, height, 
	...
	if (output == NULL) {
		sdlperror("hflip: SDL_CreateRGBSurface");
		return NULL;  // Don't quit, return NULL and let caller handle it
	}
	if (bpp == 1 && input->format->palette) {
		SDL_SurfaceAdoptPalette(output, input->format->palette);
	}
	
	// Direct pixel flip - much faster than per-column blits
	uint8_t* src_pixels = (uint8_t*)input->pixels;
	uint8_t* dst_pixels = (uint8_t*)output->pixels;
	int src_pitch = input->pitch;
	int dst_pitch = output->pitch;
	...
	for (int y = 0; y < height; ++y) {
		...
		if (bpp == 1) {
			// 8bpp indexed - flip bytes
			for (int x = 0; x < width; ++x) {
				dst_row[width - 1 - x] = src_row[x];
			}
		} else if (bpp == 4) {
			// 32bpp RGBA - flip 4-byte pixels
			uint32_t* src32 = (uint32_t*)src_row;
			uint32_t* dst32 = (uint32_t*)dst_row;
			for (int x = 0; x < width; ++x) {
				dst32[width - 1 - x] = src32[x];
			}
		} else {
			// Fallback for other formats
			for (int x = 0; x < width; ++x) {
				memcpy(dst_row + (width - 1 - x) * bpp, src_row + x * bpp, bpp);
			}
		}
	}
...
//	word image_flipped;
	...
	*/
	...
	if (chtab_flip_clip[chtab_id]) {
		...
		// RP2350: For sprites without their own clip (like falling floor tiles),
		// apply game area clip to prevent drawing into status bar (y >= 192)
	...
	if (chtab_flip_clip[chtab_id]) {
		...
		// RP2350: Reset the game area clip we set above
		reset_clip_rect();
	}
#endif
	if (need_free_image) {
...
void draw_tables() {
...
#ifdef POP_RP2350
	// Set clip rect to game area (y < 192) only during gameplay
	extern short start_level;
	if (start_level >= 0) {
		rect_type game_area_clip = {0, 0, 192, 320};
		set_clip_rect(&game_area_clip);
		if (onscreen_surface_) {
			SDL_Rect game_clip = {0, 0, 320, 192};
			SDL_SetClipRect(onscreen_surface_, &game_clip);
		}
	}
#else
	rect_type game_area_clip = {0, 0, 192, 320};
	...
	if (start_level >= 0) {
		reset_clip_rect();
		if (onscreen_surface_) {
			SDL_SetClipRect(onscreen_surface_, NULL);
		}
	}
#else
	reset_clip_rect();
...
#ifdef POP_RP2350
	// Clip dirty rect to game area (y < 192) to prevent copying garbage to health bar
	// Only apply during actual gameplay (start_level >= 0), not title/cutscenes
	extern short start_level;
	rect_type clipped = *source;
	if (start_level >= 0) {
		if (clipped.bottom > 192) clipped.bottom = 192;
		if (clipped.top >= 192) return;  // Entirely in health bar area, skip
		if (clipped.bottom <= clipped.top) return;  // Zero height after clipping
		source = &clipped;
	}
#endif
...
void draw_people() {
	...
	reset_obj_clip();
#ifdef POP_RP2350
	// Reset clip rect on onscreen surface before drawing HP (health bar is at y=194)
	if (onscreen_surface_) {
		SDL_SetClipRect(onscreen_surface_, NULL);  // Allow full surface for HP drawing
	}
#endif
	draw_hp();
#ifdef POP_RP2350
	// Re-apply game area clip after HP drawing to protect health bar
	if (onscreen_surface_) {
		SDL_Rect game_clip = {0, 0, 320, 192};
		SDL_SetClipRect(onscreen_surface_, &game_clip);
	}
