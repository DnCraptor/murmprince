...
void init_game_main() {
	...
	show_splash(); // added
	#ifdef POP_RP2350
	// Mark PSRAM session after initial load - this allows restore on restart
	psram_mark_session();
	#endif
	start_game();
}
...
void draw_game_frame() {
	if (need_full_redraw) {
		...
		if (different_room) {
			...
			// Force HP redraw when entering a different room
			hitp_delta = 1;
			if (Guard.room == next_room) {
				guardhp_delta = 1;
			}
			redraw_screen(1);
		} else {
...
void show_title() {
#ifdef POP_RP2350
	// Reset clip rect from gameplay (0-192) to allow full screen drawing (0-200)
...
void clear_screen_and_sounds() {
	...
	// RP2350: must free peels to avoid PSRAM memory leak
	free_peels();
...
void show_splash() {
	#ifdef POP_RP2350
	// RP2350 bring-up currently has no keyboard/gamepad events, and the splash screen
	// waits for "any key". Skip it to avoid an infinite wait.
	return;
	#endif
